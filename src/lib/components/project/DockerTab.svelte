<script lang="ts">
  ...
  async function doStart() {
    loading = "starting"; try { await startProject(name); } catch(e) { alert(e); } finally { loading = ""; }
  }
  async function doStop() {
    loading = "stopping"; try { await stopProject(name); } catch(e) { alert(e); } finally { loading = ""; }
  }
  async function doRestart() {
    loading = "restarting"; try { await restartProject(name); } catch(e) { alert(e); } finally { loading = ""; }
  }
...
<div class="docker-tab">
  ...
  {#if project.containers.length > 0}
    <h3>Conteneurs</h3>
    <table>
      <thead><tr><th>Service</th><th>Nom</th><th>Statut</th><th>Health</th><th>Ports</th></tr></thead>
      <tbody>
